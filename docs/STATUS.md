...
### Недавно закрыто (2026-09-18, окна отправки)

- ✅ Окна «Написать» (КП), «В список», черновик письма, «Письма всем», кампания из списка — на токенах дизайн-системы (изумрудный акцент, кнопки-пилюли, без slate/violet и классов тёмной темы); у «×» подпись «Закрыть», окончания («24 компании»), «драфт» → «черновик». Логика и доступ не менялись. Решение @user 18.09: отправка писем и КП — пока внутренний инструмент, пользователям не открываем.

### Недавно закрыто (2026-09-18, замечания по результатам поиска сайтов)

- ✅ **/runs/{id}**: блок «Условия отбора» — слова, которые должны быть на сайте, и исключения («не содержит», красным), «только с телефоном», глубина выдачи; поисковик по-русски («Яндекс XML»); колонки таблицы перераспределены — слова-совпадения не налезают на TG/VK, телефон и email читаются целиком (кнопка «копировать» — при наведении).
- ✅ **Логотип №15** (выбор @user): надпись SPINLID с изумрудной чертой под «LID» в шапках и подвале, иконка «SL» на тёмной плитке; картинка для превью ссылок `/opengraph-image` снова собирается (на проде была 502).
- ✅ **Меню**: «По боли» убран из верхнего меню и нижних вкладок — открывается переключателем «Карты · Сайты · По боли» на «Поиске»; на /app/pains подсвечен «Поиск».

### Недавно закрыто (2026-09-17, кабинет в виде Premium: PR #252 + ветка `feat/premium-company-drawer`)

- ✅ **Поиск сайтов в Яндексе и Google (#252)**: результат в кабинете вместо `/runs` — прогресс в два этапа («выдача» → «проверка сайтов»), условия запуска на виду, таблица подходящих сайтов с телефоном/email и «Выгрузить CSV»; условие не теряет слово при смене поля; «Карты / Сайты / По боли» на одной разметке (`SearchHero`). ⚠️ Бэкенд: yandex_xml при ошибке всех страниц выдачи ставит `completed` с 0 вместо `failed`.
- ✅ **Карточка компании**: шапка с «Написать» (окно КП) и «В список», вкладки Обзор / Жалобы / Отзывы / Контакты; прежние блоки (сводка, динамика боли, сравнение с нишей, фильтры отзывов, контакты по источникам, ЛПР) сохранены. `Dialog` получил проп `header`.
- ✅ **Сводка по нише**: переключатели Segmented, частые жалобы чипами, сравнение с нишей; общий график `MonthlySourceBars` (подписи текстом, читаемы на телефоне).
- ✅ **«Списки», список, «Шаблоны КП»**: `PageHeader` / `EmptyState`, правильные окончания; из списка открывается карточка компании. «История → По сайтам»: поисковик по-русски и условия запуска.
- Проверено Playwright-сценариями на подставных данных (не в репо): карточка 41/41, сводка 17/17, страницы 18/18, сайты 16/16, по боли 13/13; `next build` ок.

### Недавно закрыто (2026-09-17, UX-премиум: PR #247/#248/#249/#250)

- ✅ **Единая сетка кабинета**: 40 страниц на `PageContainer` (1232px = шапка); было 22 паттерна контейнера / 16 ширин (672–1400px). Контрольный замер на проде: 10/10 маршрутов = 1232px. `scrollbar-gutter: stable`, фикс прыжка форма↔выдача в поиске и loading-прыжка в списках.
- ✅ **Де-слоп**: 42 эмодзи → lucide-иконки; 20 градиентов убрано (mesh на auth и пустышках, градиент-текст, фиолетовый `--accent-gradient`); пилюли «ФИШКА»/«NEW» удалены; «Скоро» ×6 → единый `Badge`; один визуал бейджей (SignalPill→Badge, 73 вызова); 20 сырых `<button>` → `Button`; ~40 «Загружаю…» → «Загрузка…»; 480 замен на токены в 6 худших файлах.
- ✅ **Дизайн-стандарт `frontend/DESIGN.md`** (подключён в AGENTS.md): адаптация emilkowalski/skills (Linear) + Vercel DE + web.dev CLS — машиночитаемые правила (токены, сетка, моушен ≤300мс, фокус, anti-slop).
- ✅ **Полиш**: моушен-токены (duration/ease), глобальный `:focus-visible`, antialiased + `tabular-nums`, `100vh→dvh`, input ≥16px на мобиле.
