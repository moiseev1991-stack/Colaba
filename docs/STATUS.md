...
### Недавно закрыто (2026-09-19, контент для поиска и нейросетей)

- ✅ Раздел «Исследования» (`/issledovaniya`): общий обзор «На что жалуются клиенты малого бизнеса» + 6 ниш (салоны красоты, ветклиники, фитнес-клубы, ремонт и отделка, автошколы, кафе и пекарни). Цифры — агрегаты выгрузки 18 455 компаний из 79 городов (август 2026), без контактов и названий; цитаты обезличены; методология на каждой странице.
- ✅ Раздел «Статьи» (`/stati`): законность холодной рассылки, где взять базу компаний, клиенты для веб-студии и для SEO/маркетингового агентства, сравнение 2GIS/Яндекс.Карт/Google Maps, покупная база vs сбор, ручной поиск vs автоматизация, шаблоны холодных писем по отзывам, КП под боль клиента.
- ✅ `/o-kompanii`, `/llms.txt` (карта сайта для нейросетей), все страницы в `sitemap.xml`, колонка «Материалы» в подвале. У статей — блок «Кратко», оглавление, FAQ и разметка schema.org (Article/HowTo, FAQPage, BreadcrumbList, Dataset у исследований).

### Недавно закрыто (2026-09-19, ТЗ по формам поиска и партии КП)

- ✅ Формы поиска (карты, сайты, по боли) — на всю ширину страницы и в одной раскладке: поля в строку, «Популярные», серая панель условий, кнопка внизу слева с пояснением.
- ✅ «По боли»: поле «Своя боль» работает без ниши — ищет компании по тексту негативных отзывов (фразы через запятую); новый параметр `q` у `GET /maps/pains/companies` и его Excel-экспорта. Источник отзывов — выпадающий список.
- ✅ Партия КП: главная кнопка «Скачать таблицу: контакты и КП» — новый `GET /outreach/kp/jobs/{id}/export.xlsx` (телефон, все email, сайт, Telegram, WhatsApp, VK, боль, цитата, тема и текст КП). Отправка из кабинета — серая с пометкой «скоро» (флаг `KP_SEND_FROM_CABINET`).
- ✅ «Новая партия КП»: калькулятор выручки убран, вместо него — что получится на выходе.
- ✅ Мобильная версия: кнопки режимов, чипы и «Ещё условие» ≥40px на телефоне; фильтр «Шаблонов писем» не вылезает за экран.

### Недавно закрыто (2026-09-18, шапка поиска)

- ✅ Поиск (карты · сайты · по боли): крупный лозунг и подзаголовок убраны — сверху компактная строка «Поиск», кнопка «Как это работает» (подсказка с объяснением режима) и переключатель режимов в одну строку; форма поиска сразу в первом экране. «По боли» — одна компактная карточка: город, ниша и источник отзывов в строку, жалобы в две колонки, своя жалоба, зелёная кнопка «Показать компании» внизу.

### Недавно закрыто (2026-09-18, партии КП)

- ✅ «Новая партия КП» и страница партии — на токенах дизайн-системы: общий заголовок `PageHeader` (у партии — крошки «История · Партия #N»), калькулятор «Что выжмем из партии?» в изумруде вместо фиолетового, в таблице партии юрлицо перенесено под название (название больше не обрезается). В общем `ui/badge.tsx` обводки тонов переведены на токены — вид тот же. Логика и доступ не менялись (внутренний инструмент, решение @user 18.09).

### Недавно закрыто (2026-09-18, «По боли»: выбор ниши)

- ✅ Ниша на «По боли» — выпадающий список ниш из базы (`/maps/insights/niches`, с числом компаний, поиск, «Все ниши»; своя ниша — по введённому тексту), `components/NicheCombobox.tsx`. Раньше — поле с подсказками-заглушкой из 10 пресетов; жалобы слева пересчитываются только при выборе, а не на каждую букву.
