...
## 10. Авто-алерты здоровья интеграций (новое, 08.09.2026)

Каждые 30 минут система сама проверяет ok% внешних API за последний час
(api_call_log) и при ok%<80 (или платёжных ошибках 402/403) шлёт
**Telegram-сообщение владельцу** (бот, чат OWNER_TG_CHAT_ID).

- Анти-спам: один провайдер алертится не чаще раза в 6 часов
- Если Telegram недоступен — алерт падает в логи celery-worker (error)
- Ручной сухой прогон агрегата: см. §7 или docker exec -w /app -e PYTHONPATH=/app colaba-backend-1 python3 /tmp/alert_dry.py
- Первый живой алерт ушёл 08.09 01:00 МСК: «DaData 74.8% ok, Client error 403»

Появилось такое сообщение → проверьте соответствующий ключ/тариф по
таблице §1 плана действий (docs/планы/2026-09-07-план-по-аудиту.md).
