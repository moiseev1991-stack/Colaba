...
**Обновлено:** вс 16.08.2026, 08:20 МСК
**Для:** партнёр + агент (доступ к серверу не обязателен, но для агента — есть)

---

## 0. Текущий статус (снимок на вс 16.08, 08:17 МСК)

### Марафон

| Метрика        | Значение                                      |
| -------------- | --------------------------------------------- |
| Пройдено волн  | **282** (~1 410 поисков из 3000, ~47%)        |
| Темп           | волна за 77с / 114 компаний — лучший за забег |
| Прогноз финиша | **вс вечер** — с большим запасом до Пн 12:00  |
| Ресурсы        | load ~4, RAM ~1.9G, диск 62G — все 🟢         |

### Собрано данных (вся база, за всё время)

| Данные                           | Значение                                         |
| -------------------------------- | ------------------------------------------------ |
| Компаний всего в БД              | **45 087**                                       |
| Из них марафоном (с 15.08 вечер) | 27 787                                           |
| С телефонами                     | 10 269                                           |
| С email                          | 2 037                                            |
| С болями (AI-кластерами)         | **4 519 компаний** (19 301 связок компания↔боль) |

### Ключевая цифра для рассылок

| Готовые к КП-рассылке              | Значение  |
| ---------------------------------- | --------- |
| **Компании И с болями И с почтой** | **1 420** |

### Важно про свежие компании марафона

У 27 787 свежих компаний телефоны/почты пока 0 — **обогащение
намеренно на паузе** до Пн 09:00 (чтобы не душить марафон). После
авто-разморозки контакты начнут подтягиваться (~190 компаний/час):
первые тысячи — к Пн дню, полностью — в течение недели. Проверка
прогресса — §5, признаки этапа 2.
