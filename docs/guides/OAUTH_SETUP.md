...
> **19.09: подтверждение email стало обязательным для входа.** Пока SMTP
> не настроен, флоу работает по-старому (fail-open — чтобы не закрыть вход
> всем). Как только `SMTP_*` появляются в `/opt/colaba/.env` — регистрация
> сразу начинает требовать подтверждение: письмо при регистрации, вход
> закрыт до клика по ссылке (403 `email_not_verified` + повторная отправка).
...
- **Итог (ночь 19.09): Яндекс-вход поднят и проверен живьём** — кнопка «Войти через
  Яндекс» рендерится на проде, клик ведёт на passport.yandex.ru с корректными
  client_id/redirect_uri/state (проверено браузером). Цепочка причин была тройная:
  (1) флагов NEXT_PUBLIC_OAUTH_* не было в env Coolify; (2) Dockerfile не объявлял
  ARG для них — Coolify передаёт env как --build-arg, но без ARG значения не
  доезжают до `next build` (добавлено, #275); (3) вечерний троттлинг GitHub до ДЦ
  рвал git-клон деплоя — 6 деплоев упало, успешный пойман автопилотом (канарейка-клон
  → tinker-очередь деплоя, по кругу).
