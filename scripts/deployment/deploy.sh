...
for v in POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DB DATABASE_URL DATABASE_URL_SYNC REDIS_URL CELERY_BROKER_URL CELERY_RESULT_BACKEND SECRET_KEY CORS_ORIGINS; do
  if [[ -z "${!v}" ]]; then
