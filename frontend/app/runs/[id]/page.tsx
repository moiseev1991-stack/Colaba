...
const MODULE_META: Record<
  ModuleId,
  { label: string; icon: typeof Users; historyHref: string; mono: string }
> = {
  leads: {
    label: 'Поиск лидов',
    icon: Users,
    historyHref: '/app/leads/history',
    mono: '01 / leads',
  },
  tenders: {
    label: 'Госзакупки',
    icon: Landmark,
    historyHref: '/app/gos/history',
    mono: '02 / tenders',
  },
...
export default function RunResultsPage() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        const needsResults =
          countChanged || statusChanged || auditActive || lastCountRef.current === -1;
        ...
        if (needsResults || searchData.status === 'completed') {
          ...
          const rows: LeadRow[] = resultsData.map((r) => {
            ...
            const extra = (
              r.extra_data && typeof r.extra_data === 'object'
            ...
            const classification = extra.classification as
              { site_type?: string | null; clean_description?: string | null } | undefined;
            const allowedTypes = [
              'company',
              'catalog',
              'market',
  ...
  const searchProvider = search?.search_provider ? providerLabel(search.search_provider) : '—';
  const searchStatus = search?.status || 'unknown';
  const configError =
    search?.config && typeof search.config === 'object'
  ...
  const backgroundProgressPercent =
    results.length > 0 ? Math.round((processedCount / results.length) * 100) : 0;
  ...
  return (
    <div className="w-full max-w-[1250px] min-w-0 mx-auto px-4 md:px-6 py-6 overflow-x-hidden">
      ...
      <nav
        className="flex items-center gap-1.5 mb-4 app-mono-label flex-wrap"
      ...
      >
        ...
        <Link
          href={moduleMeta.historyHref}
          className="hover:text-[hsl(var(--accent))] transition-colors"
        >
          {moduleMeta.label.toLowerCase()}
        </Link>
        <ChevronRight className="h-3 w-3 opacity-60" />
        <span style={{ color: 'hsl(var(--text))' }}>результаты</span>
        <span
          className="ml-3 inline-flex items-center gap-1.5 px-2 py-0.5"
          style={{
      ...
      >
        ...
        <MetaCell label="источник" value={searchProvider} mono={false} />
      ...
      {/* Что искали: условия и исключения запуска (18.09 @user: «по какому ключу — вижу,
          а исключения и условия отбора — нет»). */}
      <RunConditions config={search?.config} numResults={search?.num_results ?? null} />
      ...
      {searchStatus === 'failed' && (
        ...
        >
          <h3
            className="text-small font-bold uppercase tracking-wider mb-2"
          ...
          {typeof configErrorType === 'string' && (
            <p className="mt-1 app-mono-label" style={{ color: 'hsl(var(--danger))' }}>
              тип: {configErrorType}
            </p>
          )}
        </div>
      ...
      {pollTimeout && (
        ...
        >
          ...
          <p className="text-small" style={{ color: 'hsl(var(--text))' }}>
            Обновление остановлено по тайм-ауту (5 минут). Обновите страницу или попробуйте другой
            источник.
          </p>
        </div>
      )}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-small font-bold" style={{ color: 'hsl(var(--text))' }}>
          {title}
        </span>
        <span className="app-mono-label" style={{ color: accent }}>
          {percent}%
        </span>
      </div>
      <div
        className="w-full h-1.5 overflow-hidden"
        style={{ background: 'hsl(var(--border))', borderRadius: 2 }}
      >
        <div
      ...
      <p className="mt-2 app-mono-label" style={{ color: 'hsl(var(--muted))' }}>
        {subtitle}
      </p>
    </div>
  );
...
/**
 * Условия отбора запуска из config: слова на сайте (содержит / не содержит), телефон, email,
 * связка «и» / «или», «только с телефоном» и глубина выдачи. Исключения — красным.
 */
function RunConditions({
  config,
  numResults,
}: {
  config: SearchResponse['config'] | null | undefined;
  ...
  const chips = conditions
    ...
    .map((c) => ({
      ...
      exclude: c.op === 'not_contains' || c.op === 'not_equals' || c.op === 'is_false',
    }));
  const onlyPhone = cfg.filter_phone === true && !chips.some((c) => c.text === 'есть телефон');

  return (
    <section
      aria-label="Условия отбора"
      className="mb-4 flex flex-wrap items-center gap-x-2 gap-y-2 rounded-card border border-ui-border bg-ui-surface px-4 py-3"
    >
      <span className="mr-1 text-small font-semibold text-ui-text">Условия отбора:</span>
      {chips.length === 0 && !onlyPhone ? (
        <span className="text-small text-ui-text-muted">
          без условий — показаны все сайты из выдачи
        </span>
      ) : (
        <>
          {chips.map((c, i) => (
            <span key={`${c.text}-${i}`} className="inline-flex items-center gap-2">
              {i > 0 && (
                <span className="text-xs font-semibold uppercase text-ui-text-muted">
                  {logic === 'or' ? 'или' : 'и'}
                </span>
              )}
              <span
                className={cn(
                  ...
                  c.exclude
                    ? 'bg-ui-danger/[.08] text-ui-danger'
                    : 'bg-ui-accent/[.08] text-ui-accent',
                )}
                title={
                  c.exclude ? 'Исключение: такие сайты отброшены' : 'Условие: сайт должен подходить'
                }
              >
                {c.text}
              </span>
          ...
          {onlyPhone && (
            <span className="inline-flex items-center rounded-full bg-ui-accent/[.08] px-3 py-1 text-small font-medium text-ui-accent">
              только с телефоном
            </span>
          )}
        </>
      ...
      {numResults ? (
        <span className="ml-auto text-xs text-ui-text-muted">глубина выдачи: {numResults}</span>
      ) : null}
    </section>
  );
}
