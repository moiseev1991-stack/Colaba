...
type SeoSettingKey =
  'robotsTxt' | 'sitemap' | 'duplicateTitlesDescriptions' | 'emptyTitlesDescriptions';
...
const SEO_CHECKS: { key: SeoSettingKey; label: string }[] = [
  ...
  {
    key: 'emptyTitlesDescriptions',
    label: 'Отсутствие заголовков и описаний (пустые заголовки и описания)',
  },
];
  ...
  return (
    <PageContainer className="overflow-x-hidden">
      <div className="space-y-6">
        <PageHeader
          breadcrumbs={[{ label: 'Главная', href: '/' }, { label: 'Конфигурация' }]}
          title="Конфигурация"
        />
...
function SeoCheckbox({
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer group">
      <div className="relative">
        ...
        <div
          className={`w-5 h-5 rounded-v2-sm border-2 flex items-center justify-center transition-all ${
            checked ? 'bg-brand-500 border-brand-500' : 'group-hover:border-brand-400'
          }`}
          style={
            !checked
              ? {
      ...
      <span className="text-base" style={{ color: 'hsl(var(--text))' }}>
        {label}
      </span>
    </label>
  );
