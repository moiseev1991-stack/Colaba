...
import { ArrowRight, Check, Rocket, RotateCw, Target, X, Zap } from 'lucide-react';
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-3 space-y-2 text-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <button
            ...
            disabled={parseBusy}
            className="inline-flex items-center gap-1.5 rounded-md border border-slate-900 bg-slate-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-slate-700 disabled:bg-slate-400"
          >
            {parseBusy ? (
          ...
          <div className="rounded-md border border-slate-100 bg-slate-50 p-2 text-xs space-y-0.5 max-h-40 overflow-auto">
            ...
            {parseResults.slice(-8).map((r, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                >
                  {r.status === 'pending' && <Check className="h-4 w-4" aria-hidden />}
                  {r.status === 'from_cache' && <Zap className="h-4 w-4" aria-hidden />}
                  {r.status === 'failed' && <X className="h-4 w-4" aria-hidden />}
                </span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {q && (
              <button
                type="button"
                onClick={() => setQ('')}
                className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-white px-2 py-1 text-xs text-slate-600 hover:bg-slate-100"
              >
            ...
            <button
              type="button"
              onClick={() => void reload()}
              className="ml-auto inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-white px-2 py-1 text-xs text-slate-600 hover:bg-slate-100"
            >
                    ...
                    <tr key={key} className="hover:bg-slate-50 align-top">
                      ...
                      <td className="px-3 py-2 text-right space-y-1">
                        <div className="flex flex-wrap justify-end gap-1">
                          <Link
                            href={painsHref}
                            className="inline-flex items-center gap-1 rounded-md border border-slate-300 bg-white px-2 py-0.5 text-xs text-slate-700 hover:bg-slate-100"
