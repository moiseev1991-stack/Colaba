  ...
  return (
    <div className="mx-auto max-w-[1000px] space-y-5 px-6 py-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Мои пресеты фильтров</h1>
      ...
      {loading ? (
        ...
        <ul className="divide-y divide-slate-200 rounded-md border border-slate-200">
          {visible.map((p) => (
            <li key={p.id} className="flex items-start justify-between gap-4 px-4 py-3 hover:bg-slate-50">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="truncate text-sm font-medium text-slate-900">{p.name}</span>
                  {p.ai_prompt && p.ai_prompt.trim() && (
                    <span className="inline-flex items-center gap-0.5 rounded bg-violet-100 px-1.5 py-0 text-[10px] font-semibold text-violet-800">
                      <Sparkles className="h-2.5 w-2.5" /> AI
                    </span>
                  )}
                  {p.hidden && (
                    <span className="rounded bg-slate-200 px-1.5 py-0 text-[10px] font-medium text-slate-700">
                      скрыт
                    </span>
                  )}
                </div>
                ...
                <div className="mt-1 text-[12px] text-slate-500">
                  <span className="font-medium text-slate-600">Фильтр: </span>
                  {summarizeFilter(p.filter as MapSearchFilter)}
                </div>
                {p.ai_prompt && p.ai_prompt.trim() && (
                  <div className="mt-1 line-clamp-2 text-[12px] text-violet-700/90">
  ...
  return (
    <Dialog open onClose={() => !saving && onClose()} title="Редактировать пресет">
      <div className="space-y-4 p-6">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-600">Описание (опционально)</label>
          <input
            value={description}
        ...
        <div className="rounded-md border border-slate-200 bg-slate-50/50 px-3 py-2 text-[12px] text-slate-600">
