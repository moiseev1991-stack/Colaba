...
const MOCK_KPI = { requests: 18, success: 16, errors: 2, avgTime: '3.2 с', cost: '4.30 ₽', leads: 12, domains: 45 };
const MOCK_CHART = [2, 4, 3, 5, 2, 4, 3];
const MOCK_RUNS = [
  { id: 'l1', date: '25.01 11:00', query: 'Консультации юриста', status: 'done', results: 8 },
  { id: 'l2', date: '24.01 16:22', query: 'Строительные компании', status: 'done', results: 15 },
];
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <h1 className="text-[24px] font-semibold" style={{ color: 'hsl(var(--text))' }}>Дашборд: Поиск лидов</h1>
          ...
          {MOCK_CHART.map((h, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-2">
              <div
                ...
                style={{ height: `${(h / 5) * 140}px`, backgroundColor: '#2563eb' }}
              />
        ...
        >
          {MOCK_CHART.map((h, i) => (
            <div key={i} className="flex-1 flex gap-1 items-end" style={{ height: 140 }}>
              <div className="flex-1 rounded-t-[4px]" style={{ height: `${(h / 5) * 100}px`, backgroundColor: 'hsl(142 76% 36%)' }} />
              <div className="flex-1 rounded-t-[4px]" style={{ height: `${((5 - h) / 5) * 40}px`, backgroundColor: 'hsl(var(--danger))' }} />
            </div>
          ))}
        </div>
      </div>
