...
const MOCK_KPI = { requests: 42, success: 38, errors: 4, avgTime: '2.1 с', cost: '8.20 ₽', leads: 12, domains: 89 };
const MOCK_CHART = [6, 8, 5, 9, 7, 8, 6];
const MOCK_RUNS = [
  { id: '1', date: '25.01 14:32', query: 'Сантехник Москва', status: 'done', results: 15 },
  { id: '2', date: '25.01 12:01', query: 'Ремонт окон', status: 'done', results: 22 },
  { id: '3', date: '24.01 18:45', query: 'Юрист онлайн', status: 'error', results: 0 },
];
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <h1 className="text-[24px] font-semibold" style={{ color: 'hsl(var(--text))' }}>Дашборд SEO</h1>
          ...
          {MOCK_CHART.map((h, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-2">
              <div
                ...
                style={{ height: `${(h / 9) * 140}px`, backgroundColor: '#2563eb' }}
              />
        ...
        >
          {MOCK_CHART.map((h, i) => (
            <div key={i} className="flex-1 flex gap-1 items-end" style={{ height: 140 }}>
              <div className="flex-1 rounded-t-[4px]" style={{ height: `${(h / 9) * 100}px`, backgroundColor: 'hsl(142 76% 36%)' }} />
              <div className="flex-1 rounded-t-[4px]" style={{ height: `${((9 - h) / 9) * 40}px`, backgroundColor: 'hsl(var(--danger))' }} />
            </div>
          ))}
        </div>
      </div>
