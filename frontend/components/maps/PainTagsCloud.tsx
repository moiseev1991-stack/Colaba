...
export function PainTagsCloud({ niche, city, searchId, value, onChange }: Props) {
  ...
  // text-xs — иначе в узкой боковой панели «Спорные доплаты за лечение»
    ...
    <div className="flex flex-wrap gap-1">
      {value.length > 0 && (
        <button
          type="button"
          onClick={() => onChange([])}
          className="rounded-md border border-rose-200 bg-white px-2 py-0.5 text-xs font-medium text-rose-700 hover:bg-rose-50 dark:border-rose-700/50 dark:bg-slate-900 dark:text-rose-300"
      ...
      {tags.map((t) => {
        ...
        return (
          <button
            ...
            className={cn(
              'cursor-pointer rounded-md border px-2 py-0.5 text-xs font-medium leading-snug transition-colors',
              selected
