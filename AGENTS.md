...
### ⚠️ Деплой на перегруженный сервер ЗАПРЕЩЁН

Прод — сервер с **2 ядрами / 3.8GB RAM** (self-hosted runner живёт на нём же). Инцидент 15.08: деплой во время перегруза (load 109, форк-бомба Chromium) упал и усугубил аварию, прод лежал ~30 минут.

**Перед ЛЮБЫМ деплоем или тяжёлой операцией на проде (SSH `spinlid-prod`) — обязательный pre-flight:**

```bash
ssh spinlid-prod 'uptime; nproc; free -h | grep Mem; pgrep -fc chrome || echo 0'
```

Правила по результатам (load = 1-минутный, ядра = `nproc`):

| Состояние          | Признаки                                              | Действие                                                                                                                                        |
| ------------------ | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ Норма           | load < 3× ядер, chrome < 50, RAM avail > 500MB        | Деплой разрешён                                                                                                                                 |
| 🟡 Мягкий перегруз | load 3–10× ядер ИЛИ RAM avail < 500MB                 | `docker pause colaba-celery-worker-enrich-1 colaba-celery-worker-search-1` → деплой → **обязательно** `docker unpause` (даже если деплой упал!) |
| 🔴 Инцидент        | load > 10× ядер ИЛИ chrome > 200 ИЛИ прод не отвечает | Деплой ЗАПРЕЩЁН. Сначала стабилизация по `docs/deployment/DEPLOY_RUNBOOK.md`                                                                    |

CI/CD уже защищён автоматикой (`.github/workflows/deploy.yml`, шаг `Pre-deploy load check`: пауза/abort/unpause). Эти правила — для ручных деплоев и операций агентов по SSH.

**Дополнительно:** не запускать массовые bulk-операции (50+ поисков, массовый enrich) одновременно с деплоем; после emergency `docker stop/kill` контейнеров — проверить, что следующий `docker compose up -d` не упадёт на конфликте имён (`docker rm -f` старых перед `up`).
...
- **Дизайн: `frontend/DESIGN.md` — обязательный стандарт** (токены, сетка страниц,
  компоненты, моушен ≤300мс, фокус, anti-slop правила). Перед UI-правками прочитать;
  нарушения (сырые цвета, `text-[13px]`, эмодзи, градиенты в кабинете) — баг.
