    ...
    async with AsyncSessionLocal() as db:
        ...
        co = (
            await service.save_companies_batch(
                db,
                [CompanyRaw(source="2gis", external_id=_unique_id("co"), name="X")],
                search.id,
            )
        )[0]

        # 3 отзыва: свежий, «средний» (35 дней — внутри retention) и старый
        fresh_marker = f"fresh-{uuid.uuid4()}"
        mid_marker = f"mid-{uuid.uuid4()}"
        ...
        await service.save_reviews_batch(
            ...
            [
                ...
                ReviewRaw(source="2gis", rating=5, raw_text=mid_marker),
                ReviewRaw(source="2gis", rating=5, raw_text=old_marker),
            ],
        )

        # retention = REVIEW_RAW_TEXT_RETENTION_DAYS (180 после PR #182:
        # 30 дней зануляли июльскую пачку и цитаты пропадали из UI/КП).
        # «Средний» отзыв на 35 дней — НЕ должен затираться; старый на
        # retention+20 дней — должен.
        from app.modules.maps.tasks import REVIEW_RAW_TEXT_RETENTION_DAYS

        await db.execute(
            text(
                "UPDATE reviews SET created_at = NOW() - (:d * INTERVAL '1 day') "
                "WHERE company_id = :cid AND raw_text = :marker"
            ),
            {"d": REVIEW_RAW_TEXT_RETENTION_DAYS + 20, "cid": co.id, "marker": old_marker},
        )
        await db.execute(
            text(
                "UPDATE reviews SET created_at = NOW() - INTERVAL '35 days' WHERE company_id = :cid AND raw_text = :marker"
            ),
            {"cid": co.id, "marker": mid_marker},
        )
        await db.commit()
        ...
        rows = list(
            (
                await db.execute(select(Review.raw_text, Review.raw_text_purged_at).where(Review.company_id == co.id))
            ).all()
        )
        by_text = {r[0]: r[1] for r in rows}
        # свежий и «средний» (35 дней < retention) по-прежнему имеют текст
        assert fresh_marker in by_text and by_text[fresh_marker] is None  # purged_at NULL = не трогали
        assert mid_marker in by_text and by_text[mid_marker] is None
