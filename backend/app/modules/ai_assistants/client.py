...
async def _chat_openai(model: str, cfg: dict, messages: list, max_tokens: int, temperature: float) -> str:
    ...
    # Инцидент 08.09: ProxyAPI-баланс кончился (402) — LLM-провалы не
    # попадали в api_call_log (писался только успех) → алерты слепы,
    # warmup три дня молча генерил 0 КП. Теперь провал пишется с
