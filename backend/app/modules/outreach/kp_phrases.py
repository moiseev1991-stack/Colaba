...
def benchmark_phrase(ratio: float | None) -> str:
    ...
    if ratio >= 1.5:
        ...
        return f"эта проблема у них упоминается в {rounded_str} раза чаще, чем в среднем по нише"
    if ratio < 0.66:
        return "эта проблема у них упоминается реже, чем в среднем по нише — они держат планку"
