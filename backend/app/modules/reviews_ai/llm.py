...
def _soften_pain_label(text: str) -> str:
    ...
    Регистр-нечувствительно, с сохранением капитализации первой буквы.
    Запускается на label и description после LLM-наминга.
    """
    if not text:
        return text
    lower = text.lower()
    for bad, good in _PAIN_LABEL_SOFT_MAP:
        if bad in lower:
            # Замена в нижнем регистре, потом восстанавливаем первую заглавную
            lower = lower.replace(bad, good)
    if not lower:
        return text
    # Капитализируем первую букву (как у label'ов «Цены и доплаты»).
