...
# LLM иногда выдаёт «Мошенничество», «Обман», «Развод» — это плохо смотрится
# в карточке-лиде (это, в конечном счёте, текст ДЛЯ ПРОДАЖИ услуги хозяину
# компании, к которому юзер собирается стучаться холодным письмом). Заменяем
# на нейтральные формулировки в post-process — даже если промпт не помог.
_PAIN_LABEL_SOFT_MAP: tuple[tuple[str, str], ...] = (
    # Длинные сначала, чтобы «мошенничество с ценами» не превратилось в
    # «спорные доплаты с ценами» через 2 итерации.
    ("мошенничество с ценами", "спорные доплаты"),
    ("мошенничество и завышенные цены", "жалобы на цены и допработы"),
    ("мошенничество в", "недоверие к ценам в"),
    ("обман с гарантией", "сложности с гарантией"),
    ("обман клиентов", "жалобы на коммуникацию"),
    ("развод на деньги", "спорные доплаты"),
    ("развод клиентов", "спорные доплаты"),
    ("непрофессионализм", "низкая квалификация специалистов"),
    ("непрофессиональные", "низкая квалификация специалистов"),
    ("мошенничество", "недоверие к ценам"),
    ("мошенники", "недоверие к ценам"),
    ("обман", "коммуникация и ожидания"),
    ("развод", "спорные доплаты"),
    ("жулики", "недоверие"),
    ("воры", "спорные доплаты"),
    ("кидалово", "спорные доплаты"),
    ("лохотрон", "недоверие"),
    ("преступники", "жалобы"),
)


def _soften_pain_label(text: str) -> str:
    """Заменяет обвинительные слова в pain-label на нейтральные.

    Регистр-нечувствительно. Если совпадений с блок-листом не нашлось,
    возвращает исходный текст БЕЗ изменений (важно: не трогаем регистр,
    чтобы не сломать существующее тестовое ожидание и не «исправлять»
    нормальные label'ы вроде «долгое ожидание»). Если же был хоть один
    matched-replace — финальный текст капитализируется (вид «Спорные
    доплаты» вместо «спорные доплаты»).
    """
    if not text:
        return text
    lower = text.lower()
    replaced = False
    for bad, good in _PAIN_LABEL_SOFT_MAP:
        if bad in lower:
            lower = lower.replace(bad, good)
            replaced = True
    if not replaced:
        return text
    # Капитализируем первую букву после реальной замены (label'ы вида
    # «Цены и доплаты»).
