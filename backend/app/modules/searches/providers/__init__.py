...
async def fetch_search_results(
    ...
    **kwargs,
    ...
    for current_provider in providers_to_try:
        try:
            if current_provider == "yandex_xml":
                ...
                return await yandex_fetch(
                    query=query, num_results=num_results, provider_config=provider_config, **kwargs
                )

            elif current_provider == "serpapi":
                from app.modules.searches.providers.serpapi import fetch_search_results as serpapi_fetch

                return await serpapi_fetch(
                    query=query, num_results=num_results, provider_config=provider_config, **kwargs
                )
                ...
                return await yandex_html_fetch(
                    query=query, num_results=num_results, provider_config=provider_config, **kwargs
                )
                ...
                return await google_html_fetch(
                    query=query, num_results=num_results, provider_config=provider_config, **kwargs
                )
    ...
    if last_error:
        ...
        raise ValueError(
            f"Unknown search provider: {provider}. Available: yandex_xml, yandex_html, google_html, serpapi"
        )
