    ...
    **kwargs,
    ...
    for page_num in range(pages_needed):
        try:
            ...
            proxy_overrides = (
                {k: pc.get(k) for k in ("use_proxy", "proxy_url", "proxy_list")} if pc is not None else None
            )
            ...
            if not page_results:
                if page_num == 0:
                    ...
                    raise ValueError(
                        "Google HTML: no results on first page (likely blocked or captcha). Try yandex_html or yandex_xml."
                    )
                else:
                    break
...
async def _fetch_page(
    ...
    response = await fetch_with_retry(
        url, max_retries=max_retries, timeout=30.0, referer="https://www.google.com/", proxy_overrides=proxy_overrides
    )
    ...
    if db is not None and (
        (bi.get("blocked") and bi.get("block_type") == "captcha") or response.status_code in (403, 429)
    ):
        try:
...
async def _try_submit_google_recaptcha_form(
    ...
    recaptcha_field = soup.find(attrs={"name": "g-recaptcha-response"}) or soup.find(
