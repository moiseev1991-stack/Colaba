...
async def get_current_user_id(
    ...
    Безопасность (аудит 18.09):
    - refresh-токен (type=refresh) не проходит как access (КР-1);
    - проверяем is_active в БД — блокировка юзера из админки закрывает
      сессию на следующем запросе (раньше токен жил до экспирации).
...
async def get_current_organization_id(
    ...
    result = await db.execute(
        select(user_organizations.c.organization_id).where(user_organizations.c.user_id == user_id).limit(1)
    )
    org_id = result.scalar_one_or_none()

    if not org_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User has no organizations")
...
async def require_organization_admin_or_owner(
    ...
    if role not in (OrganizationRole.OWNER, OrganizationRole.ADMIN):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Organization admin or owner access required")
