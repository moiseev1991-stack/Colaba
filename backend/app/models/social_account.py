...
class SocialAccount(Base):
    ...
    # values_callable: храним ЗНАЧЕНИЯ enum ('yandex'), а не имена ('YANDEX') —
    # PG-тип oauthprovider собран из значений; имена падали InvalidTextRepresentation.
    provider = Column(
        SQLEnum(OAuthProvider, values_callable=lambda e: [i.value for i in e]),
        nullable=False,
    )
